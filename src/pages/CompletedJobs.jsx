...
import { PaymentBadge, PaymentStatusBadge } from '../components/StatusBadge'
...
export default function CompletedJobs() {
  ...
  const load = async () => {
    ...
    const [{ data: jobsData }, { data: pays }] = await Promise.all([
      supabase.from('jobs').select('*, customers(name,contact,alt_contact)')
        .is('deleted_at', null).eq('status', 'Delivered')
  ...
  // refresh after a change in the panel; if a job is re-opened it leaves this list
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-heading font-bold text-2xl text-ink">Completed Jobs</h1>
          <p className="text-sm text-ink-300">{jobs.length} delivered · open a job to view invoice or re-open it</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input className="input pl-10" placeholder="Search completed jobs…"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-ink-50/60 text-ink-400 text-xs uppercase tracking-wide">
                ...
                <th className="text-left font-semibold px-4 py-3">Delivered On</th>
                ...
                <tr>
                  <td colSpan={8}>
                    <EmptyState icon="✅" title="No completed jobs yet"
                      message={query || payment !== 'All' ? 'Try a different search or filter.' : 'Jobs marked Delivered will appear here.'} />
                  </td>
                </tr>
              ) : (
                filtered.map((j) => (
                  <tr key={j.id} className="table-row-hover" onClick={() => setSelected(j)}>
                    ...
                    <td className="px-4 py-3 text-ink-400 whitespace-nowrap">{formatDate(j.delivered_at || j.updated_at)}</td>
